...
// The file was restored, so it does NOT have the isLoading fix.
// Let's add the isLoading fix first.
content = content.replace(
  `const { events } = useEventStore()`,
  `const { events, isLoading } = useEventStore()`
);

// Find where "return (" starts after "if (success) {" block
const returnIndex = content.indexOf('  return (\n    <div className="min-h-screen bg-gray-50 pb-16">');

const beforeReturn = content.slice(0, returnIndex);
const oldReturnContent = content.slice(returnIndex);

// Define the two layouts
const splitLogic = `
  const hackathonLayout = (
\${oldReturnContent.slice(11).trim().slice(0, -1)}
  );
...
console.log("Rewritten EventRegistrationPage.jsx successfully.");
